...
import com.example.wine.databinding.FragmentThirdTableBinding
...
class Third_table : Fragment() {

    lateinit var binding: FragmentThirdTableBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentThirdTableBinding.inflate(inflater)
    ...
    private fun setvalue(sp: SharedPreferences) = with(binding){
        ...
        var qvad = 0.0
        for(i in 1..5){
            val value = sp.getFloat("third" + (i).toString(), 0.0F)
            when(i){
                1->{binding.edFirst.text = ((value*st).roundToInt()/st).toString()
                    binding.resDelta1.text = (((value-77.0)*st).roundToInt()/st).toString()
                    binding.res1.text = ((abs(((value-77.0)/77.0)*100.0) *st).roundToInt()/st).toString()}
                2->{binding.edSecond.text = ((value*st).roundToInt()/st).toString()
                    binding.resDelta2.text = (((value-77.0)*st).roundToInt()/st).toString()
                    binding.res2.text = ((abs(((value-77.0)/77.0)*100.0) *st).roundToInt()/st).toString()}
                3->{binding.edThird.text = ((value*st).roundToInt()/st).toString()
                    binding.resDelta3.text = (((value-77.0)*st).roundToInt()/st).toString()
                    binding.res3.text = ((abs(((value-77.0)/77.0)*100.0) *st).roundToInt()/st).toString()}
                4->{binding.edFourth.text = ((value*st).roundToInt()/st).toString()
                    binding.resDelta4.text = (((value-77.0)*st).roundToInt()/st).toString()
                    binding.res4.text = ((abs(((value-77.0)/77.0)*100.0) *st).roundToInt()/st).toString()}
                5->{binding.edFifth.text = ((value*st).roundToInt()/st).toString()
                    binding.resDelta5.text = (((value-77.0)*st).roundToInt()/st).toString()
                    binding.res5.text = ((abs(((value-77.0)/77.0)*100.0) *st).roundToInt()/st).toString()}
            }
            summax+=value
            summad+=(value-77.0)
            summadl+= abs(((value-77.0)/77.0)*100.0)
        }
        ...
        for(i in 1..5){
            val value = sp.getFloat("third" + (i).toString(), 0.0F)
            if(i!=5) {
                val value1 = sp.getFloat("third" + (i + 1).toString(), 0.0F)
                qvad += ((value1-77.0) - (value-77.0)).pow(2)
            }
            ckochisl += ((value-77.0) - summad/5.0).pow(2)
            println(ckochisl)
        }
